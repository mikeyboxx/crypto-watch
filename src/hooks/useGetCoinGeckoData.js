...
export default function useGetCoinGeckoData() {
  ...
  const getData = useCallback(queryParams => {
    ...
    switch (queryParams?.reqType) {
      case 'markets':
        url = `https://api.coingecko.com/api/v3/coins/markets?vs_currency=usd&page=1&ids=${queryParams.coinId.replace(
          ',',
          '%2C'
      ...
      case 'detail':
        url = `https://api.coingecko.com/api/v3/coins/${queryParams.coinId}?developer_data=true&market_data=true&sparkline=false&community_data=true&localization=true&tickers=true`;
        break;

      default: {
        url = `https://api.coingecko.com/api/v3/coins/markets?vs_currency=usd&page=1&per_page=100&order=market_cap_desc`;
      }
    }
